...
int parsePipe(Pipe *mypipe, char* str, char** strpiped)
{
   ...
   char *bg_str = (char *)malloc(strlen(str) * sizeof(char));
   strcpy(bg_str,str);
   ...
   while (token != NULL)
   {
      ...
      char* temp_sub = strchr(token, '&');
      if (temp_sub) {
        char s[2] = " ";
        char* tempstore = (char *)malloc(512 * sizeof(char));
        strcpy(tempstore,token);
        tempstore = strtok(tempstore,s);
        temp_sub = strchr(tempstore, '&');
        int position_sub = (int)(temp_sub - tempstore);
        //fprintf(stderr,"current & located at %d\n", position_sub);
        if (position_sub == 0){
          fprintf(stderr,"Error: missing command\n");
          return 0;
        }
   ...
   // error handling of pipe sign and number of commands matching
   ...
   char *background;
   background = strchr(bg_str, '&');
   if (background != NULL){
      index = (int)(background - bg_str);
      // The background sign may only appear as the last token of a command line.
      if (index == (strlen(bg_str)-1)){
         str = strtok(bg_str,"&");
